...
static const uint8_t  SCREEN_BRIGHT   = 100;  // 7..15
...
static uint16_t signalColor(uint32_t now) {
  if (state == SHOOTING) {
    return (remainingSec(now) <= WARN_SEC) ? TFT_YELLOW : TFT_GREEN;
  }
  return TFT_RED;
}
...
static void render(uint32_t now, uint16_t value, bool blank) {
  ...
  const uint16_t fg = (bg == TFT_RED) ? TFT_WHITE : TFT_BLACK;
  ...
  if (!blank) {
    ...
    spr.drawNumber(value, W / 2, H / 2 + 1, bigFont);
  }

  // 残時間バー
  const int16_t barH = 6, barY = H - barH - 2, barX = 4, barW = W - 8;
  spr.drawRect(barX, barY, barW, barH, fg);
  if (durationSec > 0) {
    const int16_t fill = (int16_t)((uint32_t)(barW - 2) * remainingMs(now) / totalMs());
    if (fill > 0) spr.fillRect(barX + 1, barY + 1, fill, barH - 2, fg);
  }
...
void setup() {
  ...
  W = M5.Lcd.width();
  H = M5.Lcd.height();
  bigFont = (H >= 120) ? 8 : 7;  // M5StickC Plus では大きいフォントを使う
